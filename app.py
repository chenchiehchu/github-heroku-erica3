...
def job_function3():
    ...
    resultProxy = my_db.execute("CREATE TABLE IF NOT EXISTS ericachen_table (uuid text NOT NULL, time text NOT NULL, aqi text, pm25 text)")
    ...
    for item in records:
        if item['County']=='高雄市':
            ...
            # insert
            resultProxy=my_db.execute("insert into ericachen_table (uuid, time, aqi, pm25) values('%s', '%s', '%s', '%s')" %(uuid, my_time, aqi, pm25))

    # get data from db
    resultProxy=my_db.execute("select * from ericachen_table")
